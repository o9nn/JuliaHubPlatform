...
SoapyLiteXM2SDR::SoapyLiteXM2SDR(const SoapySDR::Kwargs &args)
    ...
    if (args.count("eth_mode"))
        eth_mode = args.at("eth_mode");
    if (eth_mode == "udp")
        _rx_udp_mode = SoapyLiteXM2SDREthernetMode::UDP;
    else if (eth_mode == "vrt")
        _rx_udp_mode = SoapyLiteXM2SDREthernetMode::VRT;
    else
        throw std::runtime_error("Invalid streaming mode: " + eth_mode);
    try {
        if (_rx_udp_mode == SoapyLiteXM2SDREthernetMode::UDP) {
            _rx_udp_receiver = new LiteXM2SDRUPDRx(eth_ip, "2345", 0, 20, 1024/8, 8);
        } else if (_rx_udp_mode == SoapyLiteXM2SDREthernetMode::VRT) {
            _rx_udp_receiver = new LiteXM2SDRUPDRx(eth_ip, "4991", 0, 20, 1024/8, 8);
        }
    ...
    /* Disable VRT Streaming */
    litex_m2sdr_writel(_fd, CSR_CROSSBAR_DEMUX_SEL_ADDR, 0b0);

    //uint32_t ip_addr_val = ntohl(ip_addr_struct.s_addr);

    /* Write the PC's IP to the FPGA's VRT_STREAMER IP register */
...
void SoapyLiteXM2SDR::setGain(
    ...
    SoapySDR::logf(SOAPY_SDR_DEBUG,
        "SoapyLiteXM2SDR::setGain(%s, ch%zu, %f dB)",
        dir2Str(direction),
        channel,
        value);
...
void SoapyLiteXM2SDR::setGain(
    ...
    const double value) {
    SoapySDR::logf(SOAPY_SDR_DEBUG,
        "SoapyLiteXM2SDR::setGain(%s, ch%zu, %s, %f dB)",
        dir2Str(direction),
        channel,
        name.c_str(),
        value);
...
void SoapyLiteXM2SDR::setFrequency(
    ...
    SoapySDR::logf(SOAPY_SDR_DEBUG,
        "SoapyLiteXM2SDR::setFrequency(%s, ch%zu, %s, %f MHz)",
        dir2Str(direction),
        channel,
        name.c_str(),
        frequency / 1e6);
