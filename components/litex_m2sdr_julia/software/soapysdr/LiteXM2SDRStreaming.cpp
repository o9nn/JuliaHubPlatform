...
int SoapyLiteXM2SDR::activateStream(
    ...
    if (stream == RX_STREAM) {
        ...
        if (_rx_udp_mode == SoapyLiteXM2SDREthernetMode::UDP) {
            ...
            litex_m2sdr_writel(_fd, CSR_CROSSBAR_DEMUX_SEL_ADDR, 2);
        } else if (_rx_udp_mode == SoapyLiteXM2SDREthernetMode::VRT) {
            /* Enable VRT Streaming */
...
int SoapyLiteXM2SDR::acquireReadBuffer(
    ...
    std::vector<char> vc = _rx_udp_receiver->get_data(timeoutUs);
    if (vc.empty()) {
        return SOAPY_SDR_TIMEOUT;
    }
    memcpy(ptr, vc.data(), vc.size());
    pos += vc.size();
    handle = pos;

    // get_data can read less than the buffer size, so don't return the MTU
    return vc.size() / (_nChannels * _bytesPerComplex);
