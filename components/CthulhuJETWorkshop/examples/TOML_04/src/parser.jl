...
Return a single concrete element type, if all elements of the array have the
same type. Otherwise, return Any.
"""
function static_eltype(array)
    isempty(array) && return Any

    # check for static type throughout array
    T = typeof(first(array))
    for el in array
        if typeof(el) != T
            return Any # type changed, return Any
        end
    end
    return T # concrete type matched everywhere
end
...
function parse_array(l::Parser{Dates})::Err{Vector} where Dates
    ...
    T = static_eltype(array)
    # TODO: design union-split
    #       (maybe avoid Type{T} since it's very nasty...)
    #
    #       *technically* this isn't even a union-split
    #
    # TODO: this is instead a useful example of *refinement*
    #       (ala TypeScript)
    if T === Any
        new = array
    elseif T === String
